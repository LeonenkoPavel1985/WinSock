...
int main(int argc, char* argv[])
{
	...
	hints.ai_family = AF_UNSPEC; // Семейство протоколов unspecified (не указан) можно использовать как IPv6, так и IPv4
	...
	//Закрываем сокет на отпрвку, после этого мы все еще сможем получать сообщения от сервера пока он не закроет соеденение:
	...
	do
	{
		...
		else printf("recv fauled:%d", WSAGetLastError());
	} while (iResult > 0);
