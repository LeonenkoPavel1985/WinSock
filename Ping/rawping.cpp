...
extern int setup_for_ping(char* host, int ttl, SOCKET& sd, sockaddr_in& dest)
{
	...
	if (sd == INVALID_SOCKET)
	{
		//cout - Console Out
		//cerr - Console Error
		cerr << "Invakid socket: " << WSAGetLastError() << endl;
		return -1;
	}
	...
	//Инициализируеи хост получатель.
	...
	{
		// Записываем этот адрес в получателя
	...
	else
	{
		...
		if (hp != 0)
		{
			// Есди имя разрешилось в IP-адрес, записываем его в адрес dest
...
extern int decode_reply(IPHeader* reply, int bytes, sockaddr_in* from)
{
	...
	ICMPHeader* icmphdr = (ICMPHeader*)((char*)reply + header_len); //При помощи арифметики указателей переходим с начала IP-заголовка на начало ICMP-заголовка.
