...
int main(int argc, char** argv)
{
	...
	cout << "Целевой адрес:\t" << address << endl;
	cout << "Количество запросов:\t" << count << endl;
		...
		if (send_ping(sd, dst, send_buf, packet_size) >= 0)
		{
			while (true)
			{
				if (recv_ping(sd, src, recv_buf, MAX_PING_PACKET_SIZE) < 0)
				{
					...
					ICMPHeader* icmpheader = (ICMPHeader*)((char*)recv_buf + header_len);
					if (icmpheader->seq != seq_num)
					{
						cerr << "bad seqence number!" << endl;
						continue;
					}
					else
					{
